...
export function ProductShell({ children, title = 'Home' }: { children: React.ReactNode; title?: string }) {
  ...
  return <div className="product-shell">
    <aside className="product-sidebar" aria-label="Primary navigation">
      ...
      <div className="sidebar-note"><span>Read. Keep. Remember.</span><small>Five focused minutes can change what you know.</small></div>
    </aside>
    ...
    <main className="product-main"><div className="product-topline"><div><span className="eyebrow">{title === 'Home' ? "TODAY'S LEARNING" : 'YADESH'}</span><h1>{title}</h1></div>{title !== 'You' && <Link className="search-trigger" href="/search"><Search aria-hidden="true" /><span>Search</span></Link>}</div>{children}</main>
